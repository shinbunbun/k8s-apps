...
- **Era B** (UTC 2025-10-14 〜 2026-04-21): フラット parquet、`@timestamp` (UTC ISO) と `timestamp` (JST inclusive) が両立
- **Era A** (UTC 2026-04-23 〜 2026-04-26): ネスト parquet (`json STRUCT(...)[]`)、`UNNEST(json) AS rec` が必要
- **Era C** (UTC 2026-04-27 以降): 既に JST partition、フラット parquet

`detect_era()` で UTC date から era を判定し `build_select()` で SQL を組み分ける。

### Caveats / 既知の注意点

- **出力 parquet 内に partition meta カラムが残る可能性**: 入力 parquet (Era B/C のフラット系) に `date` / `hour` / `source` カラムが含まれていた場合、`SELECT * EXCLUDE(_ts_jst)` ではこれらが残って出力される。値は元 partition (UTC) ベースなので path (JST) と齟齬が出る。読み手は **partition path を主、parquet 内部 column を副** とする運用にすること。投入前に Era B parquet 1 個を `DESCRIBE SELECT * FROM read_parquet(...)` で schema 確認推奨。
...
- **timestamp 両 NULL レコードの扱い**: `@timestamp` も `timestamp` も両方欠落するレコードは `_ts_jst::DATE = ...` で false 判定になり drop される。`event=result` の `null_ts_rows` で観測可能、想定外に多い場合は調査要。
