...
### Phase 1: 初回デプロイ (placeholder secret のまま)

PR マージ → ApplicationSet 自動検出 (namespace=scanopy) で server/daemon/CNPG が起動。
daemon は `SCANOPY_DAEMON_API_KEY=REPLACE_ME_AFTER_FIRST_LOGIN` で登録失敗するが、server は起動する。
...
### Phase 2: Scanopy 初期管理ユーザ作成

LAN 内ブラウザで http://192.168.128.16:60072 にアクセス → 初回ロード時の登録画面で管理者を作成。

### Phase 3: Daemon API key 取得

Scanopy UI > Settings > Daemons で API Key を生成 or 表示。値をコピー。

### Phase 4: Authentik OIDC Provider 作成 (手動)

Authentik 管理 UI (`https://auth.shinbunbun.com/if/admin/`) で:

1. **Providers > Create > OAuth2/OpenID Provider**
   - Name: `scanopy`
   - Client type: Confidential
   - Client ID: `scanopy` (`scanopy-secrets.oauth-client-id` と一致)
   - Client Secret: 生成値をコピー (後で secret に入れる)
...
### Phase 5: Secret 更新 + ArgoCD sync

`secrets/scanopy-secrets.enc.yaml` を編集して `daemon-api-key` と `oauth-client-secret` を実値に置き換え:

```bash
cd k8s-apps/apps/scanopy/secrets
sops scanopy-secrets.enc.yaml  # $EDITOR が開く。値を書き換えて保存すると自動再暗号化
git add scanopy-secrets.enc.yaml
git commit -m "scanopy: daemon API key / OIDC client_secret を実値で更新"
git push
```

ArgoCD が sync → daemon が再起動 → server に登録される。

### Phase 6: Scanopy 側 OIDC 有効化

Scanopy UI > Settings > Authentication > OIDC で以下を入力:
- Issuer: `https://auth.shinbunbun.com/application/o/scanopy/`
- Client ID: `scanopy`
- Client Secret: (Phase 4 で生成した値と同じもの)
